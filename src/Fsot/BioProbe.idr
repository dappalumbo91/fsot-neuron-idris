||| Biological FI metrics (host Double twin of Zig bio_probe_fixed).
...
||| Discrete FI train on 1 ms lattice.
public export
...
runFIUnit p steps =
  let dtMs = 1.0
      ...
      isis =
        if n < 2 then []
        else zipWith (\a, b => cast (b - a) * dtMs) spikeTimes (drop 1 spikeTimes)
      meanIsi = if null isis then 0 else sum isis / cast (length isis)
      rate = if steps <= 0 then 0 else cast n * 1000.0 / cast steps
      adaptIdx =
        let m = length isis
        in if m < 6 then 0
           else
             let third = max 1 (m `div` 3)
                 ...
                 me = sum early / cast (length early)
                 ml = sum late / cast (length late)
             in if me + ml < 1e-9 then 0 else (ml - me) / (ml + me)
  in MkFiResult rate meanIsi adaptIdx n

specIsiTol : Double -> Double
specIsiTol isi = if isi < 35 then max 10.0 (0.28 * isi) else max 8.0 (0.14 * isi)

||| Soft polish toward specimen ISI (mapSpecimen-style seed + iterate).
public export
...
polishToSpecimen p0 sp steps =
  let isi = max 8.0 (min 220.0 sp.spIsiMs)
      ...
      ref0 = max refLo (min 160 (the Int (cast (isi * refScale))))
      ...
      pSeed = { upRefSteps := ref0, upFiStim := fi0, upAdaptGain := g0
              , upAdaptStep := d0, upAdaptDecay := 0.988 } p0
      ...
      go it p =
        if it >= 72 then p
          ...
          in if pr.fiSpikes < 6
               then go (it + 1)
                      ({ upRefSteps := max refLo (p.upRefSteps - 2)
                       , upFiStim := min stimHi (p.upFiStim * 1.08) } p)
               else
                 ...
                 in if isiErr <= isiTol && adErr <= 0.05 then p
                      ...
                      let p1 =
                            if isiErr > isiTol && pr.fiMeanIsiMs > 1
                              then if pr.fiMeanIsiMs > sp.spIsiMs
                                     then { upRefSteps := max refLo (p.upRefSteps - 1)
                                          , upFiStim := min stimHi (p.upFiStim * (if fast then 1.05 else 1.03)) } p
                                     else { upRefSteps := min 180 (p.upRefSteps + 1)
                                          , upFiStim := max 0.28 (p.upFiStim * 0.97) } p
                              else p
                          p2 =
                            if adErr > 0.05
                              then if pr.fiAdapt < sp.spAdapt
                                     then { upAdaptStep := min 12.0 (p1.upAdaptStep * 1.12)
                                          , upAdaptGain := min 0.14 (p1.upAdaptGain * 1.06) } p1
                                     else { upAdaptStep := max 0.04 (p1.upAdaptStep * 0.90)
                                          , upAdaptGain := max 0.012 (p1.upAdaptGain * 0.95) } p1
                              else p1
                      in go (it + 1) p2
  in go 0 pSeed
